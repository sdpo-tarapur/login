...
export const DailyReportMessages: React.FC<DailyReportMessagesProps> = ({
  ...
  // Changed from single string to an array for multi-recipient selection
  ...
  // Toggle recipient selection for checkboxes
  const handleToggleRecipient = (id: string) => {
    ...
    // If selecting a specific user, remove 'ALL' if it was present
    let updated = selectedRecipientIds.filter((item) => item !== 'ALL');
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    // Send a message object for each selected recipient
    selectedRecipientIds.forEach((recipientId) => {
      let recipientName = 'All Stations & Desks (Broadcast)';
      
      if (recipientId !== 'ALL') {
        const match = userAccounts.find((a) => a.userId === recipientId || a.id === recipientId);
        if (match) {
          recipientName = `${match.officerName} (${match.rank || match.role})`;
        } else {
  ...
  return (
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl shadow-xs overflow-hidden">
      ...
      {/* Compose View with Multi-User Checkbox Selection */}
      {activeTab === 'compose' && (
        <form onSubmit={handleSend} className="p-5 space-y-4 text-xs">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              {/* Checkbox Container for Multiple Registered Users */}
              <div className="max-h-48 overflow-y-auto p-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-lg space-y-2">
                <label className="flex items-center gap-2 font-bold text-slate-900 dark:text-white pb-1 border-b border-slate-200 dark:border-slate-700 cursor-pointer">
                  <input
                    type="checkbox"
                    checked={selectedRecipientIds.includes('ALL')}
                    onChange={() => handleToggleRecipient('ALL')}
                    className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                  />
                  <span>📢 Broadcast to All Stations / General</span>
                </label>

                {userAccounts.map((acc) => {
                  const isChecked = selectedRecipientIds.includes(acc.userId);
                  return (
                    <label
                      key={acc.id}
                      className="flex items-center gap-2 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700/50 p-1 rounded cursor-pointer"
                    >
                      <input
                        type="checkbox"
                        checked={isChecked}
                        onChange={() => handleToggleRecipient(acc.userId)}
                        className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"
                      />
                      <span className="font-semibold">{acc.officerName}</span>
                      <span className="text-[10px] text-slate-400">
                        ({acc.policeStation} - {acc.rank || acc.role})
                      </span>
                    </label>
                  );
                })}
              ...
              <p className="text-[10px] text-slate-400 mt-1">
                You can select multiple registered users to send this message simultaneously.
              </p>
            </div>
                    ...
                    <div className="pt-2 border-t border-slate-100 dark:border-slate-700/60 flex items-center justify-between text-[11px] text-slate-500 flex-wrap gap-2">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {!isReadOnly && (
                          <button
                            onClick={() => {
                              setSelectedRecipientIds([msg.senderUserId]);
